...
const Basket = () => {
	const { products, items, total, clearCart, reduce } = useBasketStore();
			...
			{!order && (
				<>
					<FlatList
						...
						renderItem={({ item }) => (
							<SwipeableRow onDelete={() => reduce(item)}>
								<View style={styles.row}>
									...
									<Text onPress={() => reduce(item)}>Remove</Text>
								</View>
							</SwipeableRow>
						)}
						ListFooterComponent={
							<View>
								...
								<View style={styles.totalRow}>
									...
									<Text style={{ fontSize: 18, fontWeight: "bold" }}>
										€{(total + FEES.delivery + FEES.delivery).toFixed(2)}
									</Text>
								</View>
							</View>
						}
